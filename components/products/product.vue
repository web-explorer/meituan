<template>
  <dl class="s-item">
    ...
    <dd>
      <h3>
        <a :href="`/detail?city=${$store.state.geo.position.city}&province=${$store.state.geo.position.province}&keyword=${meta.name}&type=${meta.module}`">{{ meta.name }}</a>
<!--        <a :href="{path:'detail',query:{keyword:meta.name,type:meta.module}}">{{ meta.name }}</a>-->
      </h3>
      <el-rate
        v-model="meta.rate"
